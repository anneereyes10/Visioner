...
if (!empty($_GET['e'])) {
  $email = $_GET['e'];
  $code = $_GET['c'];
  $ucode = $clsUser->GetCodeByEmail($email);
  if ($code == $ucode) {
    if (!empty($_POST['pword']) && !empty($_POST['cpword'])) {
      if ($_POST['pword'] == $_POST['cpword']) {
        $clsUser->UpdatePasswordByEmail($email,$_POST['pword']);
        $msg = '<div class="alert alert-success">
                  <strong>Success!</strong> Password has been changed.
                </div>';
                  header('Location: ../index.php?login=&rp=1');
      } else {
        $msg = '<div class="alert alert-danger">
                  <strong>Error!</strong> Password and Confirm Password Did not match.
                </div>';
      }
    }
  } else {
...
<body>
  ...
  <!-- End of nav bar -->
    ...
    <div class="properties-area recent-property" style="background-color: #FFF;">
      <div class="container">
        <form action="?e=<?php echo $email;?>&c=<?php echo $code;?>" method="post">
